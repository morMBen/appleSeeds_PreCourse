...
const getAllAnswers = async (req, res) => {
    ...
    await usersModel.Users.findOne({passportId: _id}).populate({
        path: 'answers'
    }).exec((err, user) => {
...
const addAnswer = async (req, res) => {
    ...
    usersModel.Users.findById(req.user._id).populate({
        path: 'answers'
    }).exec((err, user) => {
        if (!user || !user.answers) return res.send(404).send("user or answers not founds");

        let answer = user.answers.find(a => a.questionID == questionId);
        let light = testsPassed === totalTests ? 2 : 1;
        if (!answer) {
            ...
            answerModel.Answer.findByIdAndUpdate(answer._id,
                {
                    lastCodeAnswer: lastCodeAnswer,
                    testsPassed,
                    trafficLight: light,
                    $inc: {clicked: 1}
                }, {new: true}, (err, doc) => {
